 ...
 *
 * - generatePostDetails - A function that generates various details for an Instagram post based on a user-provided niche and category.
...
const GeneratePostDetailsOutputSchema = z.object({
  ...
  headlineText: z.string().describe('A short, bold, and engaging headline (3-7 words) for the image, highly relevant to the niche and category, suitable for an Instagram post graphic.'),
  imageGenerationPrompt: z.string().describe('The fully constructed image generation prompt using the universal template, user niche, user category, and generated headline.'),
});
...
const universalImagePromptTemplate = "A high-quality, visually engaging Instagram post background related to the niche {NICHE} and category {CATEGORY}. The image includes a large, bold headline text that says: \"{HEADLINE}\" in modern typography, centered or creatively placed on the design. The overall composition is clean and attention-grabbing, with space for branding or additional elements. Use vibrant or niche/category-appropriate colors, gradient overlays, and subtle textures. The design follows Instagram 1:1 square format. Ensure the visual is aesthetic, scroll-stopping, and suited for social media engagement.";

const postDetailsSystemPrompt = ai.definePrompt({
  name: 'generatePostDetailsPrompt',
  input: {schema: GeneratePostDetailsInputSchema},
  output: {schema: z.object({ // AI only outputs these, imageGenerationPrompt is constructed in the flow
...
Your task is to generate comprehensive details for an Instagram post based on a user-specified niche and category.

User-provided Niche: "{{{userNiche}}}"
User-provided Category: "{{{userCategory}}}"

Based on this niche AND category, please generate the following:
...
5.  **Headline Text**: A short, bold, and engaging headline (3-7 words) that would be suitable to display prominently on an Instagram post image. This headline should be directly related to the "{{userNiche}}", "{{userCategory}}", and the generated captions. For example, if the niche is "Minimalist Home Office Setup" and category is "Productivity", a headline could be "Focus & Flow" or "Simplify Your Workspace."

Ensure your output strictly adheres to the defined output schema. Do NOT generate the full image prompt or the category itself (as it's user-provided), only the headlineText and other text details.
`,
});
...
const generatePostDetailsFlow = ai.defineFlow(
  ...
  async (input: GeneratePostDetailsInput) => {
    ...
    const finalImagePrompt = universalImagePromptTemplate
      ...
      .replace('{CATEGORY}', input.userCategory)
